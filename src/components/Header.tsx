...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-slate-50 border-b border-slate-200 text-slate-900 sticky top-0 z-40 shadow-xs backdrop-blur-md">
      ...
      <nav className="px-2 flex items-center gap-1 overflow-x-auto border-t border-slate-200 scrollbar-none py-1 bg-slate-50">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[11px] whitespace-nowrap transition-all cursor-pointer border ${
                isActive
                  ? (tab as any).isSpecial
                    ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold border-purple-600 shadow-2xs'
