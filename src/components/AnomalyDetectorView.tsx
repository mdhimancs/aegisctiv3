...
export const AnomalyDetectorView: React.FC<AnomalyDetectorViewProps> = ({
  ...
  return (
    <div className="space-y-4 text-slate-900 font-sans">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 shadow-xs flex flex-wrap items-center justify-between gap-3 font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            {filteredAnomalies.map((anom) => {
              ...
              return (
                <div
                  ...
                  className={`bg-slate-50 border rounded-lg p-2 transition-all cursor-pointer shadow-xs ${
                    isSelected
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2">
                      {/* Anomaly Score Badge */}
                      <div className="px-2 py-0.5 rounded-lg bg-red-50 border border-red-200 text-center font-mono">
                        <span className="text-[9px] text-red-700 block uppercase leading-none font-bold">SCORE</span>
                        <span className="text-xs font-black text-red-900">{anom.anomalyScore}</span>
                      </div>
                      ...
                      <div>
                        <div className="flex items-center gap-1 flex-wrap">
                          ...
                          <span
                            className={`px-1.5 py-0.2 rounded text-[9px] font-mono uppercase border ${getStatusBadge(
                              anom.status
                            )}`}
                          >
                            {anom.status.replace('_', ' ')}
                          </span>
                          <span className="px-1 py-0.2 rounded bg-indigo-50 text-indigo-700 text-[9px] font-mono font-semibold border border-indigo-200">
                            Z = +{anom.zScore.toFixed(2)}σ
                          </span>
                        </div>
                        <h3 className="text-xs font-bold text-slate-900 mt-0.5 font-sans">
                          {anom.title}
                        </h3>
                      </div>
                    </div>
                  ...
                  {/* Entity Box */}
                  <div className="mt-2 bg-slate-50 border border-slate-200 rounded-lg p-2 space-y-0.5 text-[10px]">
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-slate-500 uppercase font-mono">
                        Target / Entity:
                      </span>
                    ...
                    <div className="text-[10px] text-slate-600 font-sans truncate">
                      <span className="font-semibold text-slate-500 font-mono">Deviation: </span>
                  ...
                  <div className="flex items-center justify-between mt-2 pt-1.5 border-t border-slate-100 text-[10px]">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="px-2 py-0.5 rounded bg-red-50 hover:bg-red-100 text-red-700 text-[10px] font-semibold border border-red-200 cursor-pointer"
                      >
                        Confirm
                      </button>
                      <button
                        ...
                        className="px-2 py-0.5 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-medium border border-slate-200 cursor-pointer"
                      >
                        Baseline
                      </button>
                    </div>
        ...
        {/* Right: Selected Anomaly Dossier & AI Evaluator (6 cols) */}
        <div className="lg:col-span-6 space-y-3 font-mono">
          {selectedAnomaly ? (
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 shadow-xs space-y-3 sticky top-20">
                  ...
                  {aiResult.investigationQueries && (
                    <div className="space-y-2 pt-2 border-t border-indigo-100">
                      ...
                      {aiResult.investigationQueries.kql && (
                        <div className="bg-slate-100 text-indigo-950 p-2.5 rounded-lg text-xs font-mono relative border border-slate-300">
                      ...
                      {aiResult.investigationQueries.splunk && (
                        <div className="bg-slate-100 text-emerald-950 p-2.5 rounded-lg text-xs font-mono relative border border-slate-300">
                          <div className="flex items-center justify-between text-[11px] text-slate-500 mb-1">
