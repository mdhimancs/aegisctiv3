    ...
    <div className="space-y-6 text-slate-900 font-sans">
      ...
      {activeFeedSubTab === 'stream' ? (
        <>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 shadow-xs font-mono">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-mono text-slate-500">
                {customSources.length} Feeds Synced
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
              {customSources.map((source) => (
                <div
                  key={source.id}
                  className="bg-slate-50 border border-slate-200 rounded-lg p-2.5 flex flex-col justify-between space-y-1 hover:border-slate-400 transition-colors"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-bold text-slate-900 line-clamp-1">{source.name}</span>
                    <span className="px-1 py-0.2 rounded bg-indigo-50 border border-indigo-200 text-[9px] font-mono font-semibold text-indigo-700">
                      {source.format}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-[10px] text-slate-500">
                    <span>{source.indicatorsCount.toLocaleString()} IOCs</span>
                    <span className="font-mono text-[10px] text-emerald-700 font-medium">Every {source.pollingIntervalMin}m</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 shadow-xs flex flex-wrap items-center justify-between gap-3 font-mono">
            ...
            <div className="flex items-center gap-2 text-xs flex-wrap">
              <select
                ...
                className={`border rounded-xl px-2.5 py-1.5 text-xs focus:outline-none transition-all cursor-pointer font-medium ${
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                {filteredFeeds.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      className={`bg-slate-50 border rounded-xl p-3.5 transition-all cursor-pointer shadow-xs flex flex-col justify-between group relative overflow-hidden ${
                        isSelected
                          ? 'border-red-500 ring-2 ring-red-500/20 bg-red-50/10 shadow-md translate-y-[-2px]'
                          : 'border-slate-200 hover:border-slate-400 bg-slate-50 hover:bg-slate-50/80 hover:shadow-sm'
                      }`}
                    >
                      ...
                      <div className="space-y-2">
                        <div className="flex items-start justify-between gap-2 pt-1">
                          <div className="flex items-center gap-1.5 font-mono flex-wrap">
                            <span
                              className={`px-1.5 py-0.2 rounded text-[9px] font-bold uppercase tracking-wider ${getProviderLogoColor(
                                item.provider
                              )}`}
                            >
                              {item.provider}
                            ...
                            <span
                              className={`px-1.5 py-0.2 rounded text-[9px] font-mono font-bold uppercase border ${getSeverityBadge(
                                item.severity
                              )}`}
                            >
                              {item.severity}
                            ...
                            <span
                              className={`px-1 py-0.2 text-[9px] font-mono font-bold rounded ${getTlpBadge(
                                item.tlp
                              )}`}
                            >
                              {item.tlp}
                        ...
                        <h3 className="text-xs font-bold text-slate-900 line-clamp-2 font-mono group-hover:text-red-700 transition-colors">
                          {item.title}
                        </h3>

                        {/* Indicator MISP Chip */}
                        <div className="bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 flex items-center justify-between font-mono text-[10px]">
                          ...
                          <span className="px-1.5 py-0.5 rounded bg-amber-50 text-amber-800 font-bold border border-amber-200 text-[9px]">
                      ...
                      <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-200 text-[10px] font-mono">
                        <div className="flex flex-wrap gap-1">
                          {(item.tags || []).slice(0, 2).map((tag, idx) => (
                            <span
                              key={idx}
                              className="px-1.5 py-0.2 rounded bg-slate-100 text-slate-600 text-[9px] border border-slate-200"
                            >
                              #{tag}
                            </span>
                          ))}
            ...
            <div className="lg:col-span-5 space-y-3">
              {selectedFeedItem ? (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 shadow-xs space-y-3 sticky top-20 font-mono">
                  <div className="flex items-start justify-between gap-2 pb-2 border-b border-slate-200">
                    <div>
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`px-1.5 py-0.2 rounded text-[10px] font-bold uppercase ${getProviderLogoColor(
                      ...
                      <h3 className="text-xs font-bold text-slate-900 mt-1 leading-snug font-sans">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-lg p-2.5 space-y-1.5">
                    ...
                    <div className="font-mono text-[11px] text-indigo-700 font-bold break-all select-all bg-slate-50 p-2 rounded-lg border border-slate-200">
                    ...
                    <div className="grid grid-cols-3 gap-1.5 pt-0.5 text-center font-mono">
                      <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                        ...
                        <span className="text-[10px] font-bold text-slate-800 uppercase">{selectedFeedItem.indicatorType}</span>
                      </div>
                      <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                        <span className="text-[9px] text-slate-500 block uppercase">Conf.</span>
                        <span className="text-[10px] font-bold text-emerald-700">{selectedFeedItem.confidence}%</span>
                      </div>
                      <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                        <span className="text-[9px] text-slate-500 block uppercase">Cat.</span>
                        <span className="text-[10px] font-bold text-amber-700 uppercase">{selectedFeedItem.threatType}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-700 leading-relaxed bg-slate-50 p-2.5 rounded-lg border border-slate-200 font-sans">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {selectedFeedItem.tags.map((t, idx) => (
                        <span
                          key={idx}
                          className="px-1.5 py-0.2 rounded bg-slate-100 border border-slate-200 text-[10px] font-mono text-slate-700"
                        >
                          #{t}
                        </span>
                      ))}
                  ...
                  {selectedFeedItem.rawPayload && (
                    <div>
                      ...
                      <pre className="bg-slate-100 text-slate-800 p-3 rounded-xl text-[11px] font-mono overflow-x-auto leading-relaxed max-h-36 border border-slate-300">
                        {selectedFeedItem.rawPayload}
                      </pre>
                    </div>
                  )}
        ...
        <div className="space-y-4 animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div className="lg:col-span-5 bg-slate-50 border border-slate-200 rounded-xl p-3.5 shadow-xs space-y-3 flex flex-col justify-between font-mono">
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2.5 space-y-1.5">
                  ...
                  <p className="text-[10px] text-slate-600 leading-relaxed font-sans">
                    National cyber posture remains at <strong className="text-amber-800 font-bold">Condition Yellow (Elevated)</strong> following increased state-sponsored network scanning against critical municipal utilities.
                  </p>
                  <div className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-ping" />
                    <span className="text-[10px] text-amber-800 font-bold font-mono">Active since August 2026</span>
                  </div>
                </div>

                <div className="space-y-1.5">
                  <h3 className="text-[11px] font-bold text-slate-800 uppercase flex items-center gap-1.5">
                    <Database className="w-3 h-3 text-indigo-600" />
                    <span>STIX/TAXII Trust-Group Peers</span>
                  </h3>
                  <div className="divide-y divide-slate-200 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-0.5 text-[10px]">
                    ...
                    <div className="flex items-center justify-between py-1.5">
                      <span className="font-semibold text-slate-700">Shadowserver Node</span>
                      <span className="text-emerald-700 font-bold flex items-center gap-1">
                    ...
                    <div className="flex items-center justify-between py-1.5">
                      <span className="font-semibold text-slate-700">NATO Cyber Defence (CCDCOE)</span>
                      <span className="text-indigo-600 font-bold flex items-center gap-1">
                        <span className="w-1.5 h-1.5 bg-indigo-600 rounded-full" /> Local
                      </span>
                    </div>
                    <div className="flex items-center justify-between py-1.5">
              ...
              <div className="pt-3 border-t border-slate-150 space-y-1.5">
                <button
                  ...
                  className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-400 text-white rounded-lg text-[11px] font-bold transition-colors cursor-pointer flex items-center justify-center gap-1.5 border border-indigo-500/30"
                >
                ...
                <div className={`p-2 rounded-lg border text-[10px] leading-relaxed transition-colors ${
            ...
            <div className="lg:col-span-7 bg-slate-50 border border-slate-200 rounded-xl p-3.5 shadow-xs flex flex-col font-mono">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200 mb-3">
                <span className="text-[11px] font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                  <Flame className="w-3.5 h-3.5 text-red-600" />
                  <span>National Alert & Mitigation Bulletin</span>
                </span>
                <span className="text-[10px] text-slate-500">CISA-AIS Classified</span>
              </div>
              ...
              <div className="space-y-2 overflow-y-auto max-h-[320px] pr-1">
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="px-1.5 py-0.2 rounded bg-red-100 text-red-800 border border-red-200 text-[10px] font-bold">
                      CISA AA26-238A
                    </span>
                    <span className="text-[10px] text-slate-400">Aug 2026</span>
                  </div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-0.5 line-clamp-1 leading-relaxed">
                    Identifying novel OT malware clusters targeting programmable logic controllers (PLCs).
                  </p>
                </div>
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="px-1.5 py-0.2 rounded bg-amber-100 text-amber-800 border border-amber-200 text-[10px] font-bold">
                      SHADOWSERVER
                    </span>
                    <span className="text-[10px] text-slate-400">Aug 2026</span>
                  </div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-0.5 line-clamp-1 leading-relaxed">
                    Mass scanning detected from hijacked residential routers.
                  </p>
                </div>
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="px-1.5 py-0.2 rounded bg-indigo-100 text-indigo-800 border border-indigo-200 text-[10px] font-bold">
                      SANS ISC
                    </span>
                  ...
                  <p className="text-[10px] text-slate-500 mt-0.5 line-clamp-1 leading-relaxed">
                    Advanced actors abusing misconfigured token replay methods in cloud enterprise software.
                  </p>
                </div>
              </div>
            </div>
